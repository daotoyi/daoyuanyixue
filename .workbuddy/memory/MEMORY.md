...
- **轮播/多图展示套路 (08-31, 反复调整三次后的最终结论)**:
  1. **先问清"比例从哪来"**: 跟随原图比例 vs **固定比例(如 16:9)**。
     跟随原图时若取"最大比例"作基准, **只要有竖图就会把板块撑成方形** → 用户最终选了固定 16:9
  2. `mode` 二选一: **`aspectFill`** = 等比缩放填满容器、溢出裁切、不变形(要"等高/固定比例"用它);
     **`aspectFit`** = 完整显示但会留白(要"不裁切"用它)
  3. 容器宽度用 `uni.createSelectorQuery().select(sel).boundingClientRect()` 实测
     (⚠️ **必须包 `nextTick`**, 数据赋值后元素尚未渲染, 立即测量拿不到宽度)
  4. 高度 = (容器宽 / 并排张数 − 间隙) × 比例; 并排 N 张用 `:display-multiple-items="N"`
     ⚠️ **16:9 + 并排 2 张会相当矮**(375px 屏上单张仅约 95px 高), 想要更大就改成并排 1 张
  5. 若改为固定比例, **记得删掉不再需要的 `@load` 取尺寸逻辑**, 别留死代码

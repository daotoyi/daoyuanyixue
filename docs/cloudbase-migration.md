...
## 七、成本对比：腾讯云 CloudBase vs 微信云开发（决策参考 2026-08）

底层同源（微信云开发 = CloudBase 微信版），定价基本一致：

| 项目 | 腾讯云 CloudBase | 微信云开发 |
|---|---|---|
| 免费环境 | 1 个（开发期免费，发布后转付费） | 1 个（开发期免费，发布后 15 天转付费） |
| 基础套餐 | 个人版 19.9 元/月（20万次/2GB/500QPS） | 基础套餐 19.9 元/月（同配额） |
| 云函数资源 | 0.00011108 元/GBs | 0.00011108 元/GBs |
| CDN 流量 | 0.21 元/GB | 0.21 元/GB |
| 静态网站 | 0.00394 元/GB/天 | 0.005 元/GB/天 |
| HTTP 网关 | ✅ | ✅ |

**结论**：成本打平；微信云开发因与小程序同主体胜出（绑定/支付/登录零障碍）。
**建议**：迁移目标选微信云开发环境；正式发布后按 19.9 元/月基础套餐起步，流量上来再升级。
